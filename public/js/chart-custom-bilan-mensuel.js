...
var reclamation_non_traiter_superviseur;
var reclamation_traiter_superviseur;
var reclamation_traiter_fonder_superviseur;
var reclamation_traiter_non_fonder_superviseur;
var reclamation_en_cours;
...
xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) 
    {
        ...
        reclamation_non_traiter_superviseur=0;
        reclamation_traiter_superviseur=0;
        reclamation_traiter_fonder_superviseur=0;
        reclamation_traiter_non_fonder_superviseur=0;
        reclamation_en_cours=0;
        ...
        for(var i=1;i<services.length;i++)
        {
            ...
            {
                d = getDate(reclamation[j].daterecprec);

                actual_date = new Date();

                mois = "0"+(actual_date.getMonth()+1);


                an = actual_date.getFullYear();

                if(((d[1]==mois) && (d[2]==an)) && (reclamation[j].idserv==services[i].idserv))
                {
        ...
        setTimeout(function(){
            ...
            var ctx_mensuel = document.getElementById("bilan_mensuel_unite_traitante").getContext('2d');
            var myChart_mensuel = new Chart(ctx_mensuel, {
                type: 'bar',
                    data: {
            ...
            echarts.init(document.getElementById('reponse_superviseur_bilan_mensuel')).setOption({
        ...
        echarts.init(document.getElementById('etat_superviseur_bilan_mensuel')).setOption({
            tooltip : {
                trigger: 'item',
                formatter: '{a} <br/>{b} : {c} ({d}%)'
            },
